...
with db_session_scope() as db_session:
    ...
    if view == "📈 總覽":
        ...
        if not usage_data:
            ...
            _frag = getattr(st, "fragment", lambda f: f)

            @_frag
            def _dash_usage_line_charts():
                st.markdown("#### CPU 使用趨勢 (節點小時)")
                use_sample = st.checkbox(
                    "趨勢圖使用抽樣預覽（僅繪圖降載；KPI 與上方面板仍為完整區間聚合）",
                    value=False,
                    key="dash_usage_line_sample",
                )

                def _maybe_sample_plot(df):
                    if df is None or df.empty:
                        return df
                    if not use_sample or len(df) <= 400:
                        return df
                ...
                if not cpu_usage_df.empty:
                    cpu_plot_df = _maybe_sample_plot(cpu_usage_df)
                    if use_sample and len(cpu_plot_df) < len(cpu_usage_df):
                        st.caption(f"圖表顯示 {len(cpu_plot_df)} / {len(cpu_usage_df)} 個資料點（抽樣）。")
                    cpu_chart = (
                ...
                if not gpu_usage_df.empty:
                    gpu_plot_df = _maybe_sample_plot(gpu_usage_df)
                    if use_sample and len(gpu_plot_df) < len(gpu_usage_df):
                        st.caption(f"圖表顯示 {len(gpu_plot_df)} / {len(gpu_usage_df)} 個資料點（抽樣）。")
                    gpu_chart = (
